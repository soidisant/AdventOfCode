...
import adventofcode.comon.GridByColumns
import adventofcode.comon.Point
import adventofcode.comon.puzzleInputBufferedReader

class RegolithReservoir(val sandSource: Point.Fixed, val bottomLess: Boolean) {
    ...
    fun print() {
        grid.println(Point.Fixed(grid.minX, 0), Point.Fixed(grid.maxX, grid.maxY)) { element ->
            when (element) {
                Element.Rock -> "#"
                Element.Sand -> "o"
                Element.Air -> "."
...
fun part1() {
    val regolithReservoir = RegolithReservoir(Point.Fixed(500, 0), true)
...
fun part2() {
    val regolithReservoir = RegolithReservoir(Point.Fixed(500, 0), false)
